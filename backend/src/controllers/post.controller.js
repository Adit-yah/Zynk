...
export async function createPostController(req, res) {
  ...
  const mention = req.body.mention ? req.body.mention.split(",").map( m => m.trim()) : null;
  const file = req.file;
  ...
  try {
    ...
    const [image, mentionsUser] =  await Promise.all([
      uploadImage(file, uuidv4()),
      mention ? findUsers(mention) : null ,
    ]);
    ...
    const post = await createPost({
      ...
      image: image.url,
      caption,
      mentions: mentionUser.length ? mentionsUser.map(user => user._id) : [],
    });
